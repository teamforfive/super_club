<template>
 ...
 <div>
    <el-table
      ...
      <el-table-column align="center"   label="性别编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Sex_Code}}
        </template>
      </el-table-column>
       <el-table-column align="center"   label="性别名称" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Sex_Name }}
        </template>
      </el-table-column>
      ...
      <el-form label-width="140px" label-position="left" ref="form" :model="form" :rules="rules">
        ...
        <el-form-item label="性别名称" prop="Sex_Name">
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_Sex'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  data() {
    return {
      ...
      value:'',
       rules: {
        Sex_Name: [{ required: true, message: '性别名称必填！', trigger: 'blur' }],
    ...
    var _this=this;
      ...
      getList().then(response => {
        ...
        if (search) {
          ...
          if(this.optionValue=="Sex_Name")
          {
              if(item.Sex_Name.includes(search))
              {
                return item;
              }
              }else if(this.optionValue=="Sex_Code"){
                   var Snumber=item.Sex_Code.toString();
