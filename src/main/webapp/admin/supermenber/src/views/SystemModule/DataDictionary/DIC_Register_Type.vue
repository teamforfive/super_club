<template>
 ...
 <div>
    <el-table
      ...
      <el-table-column align="center"   label="挂号类别编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Register_Type_Code}}
        </template>
      </el-table-column>
       <el-table-column align="center"   label="挂号类别名称" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Register_Type_Name }}
        </template>
      </el-table-column>
      ...
      <el-form label-width="140px" label-position="left" ref="form" :model="form" :rules="rules">
        ...
        <el-form-item label="挂号类别名称" prop="Register_Type_Name">
          <el-input  v-model="form.Register_Type_Name" ></el-input>
        </el-form-item>
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_Register_Type'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  data() {
    return {
      ...
      dialogType: 'yes',
      options: [{
          value: 'Register_Type_Name',
          label: '挂号类别名称'
        }, {
          value: 'Register_Type_Code',
          label: '挂号类别编码'
        },],
        search:'',
        optionValue:'',
      ...
      value:'',
       rules: {
        Register_Type_Name: [{ required: true, message: '挂号类别名称必填！', trigger: 'blur' }],
    ...
    var _this=this;
      ...
      getList().then(response => {
        ...
        if (search) {
          ...
          if(this.optionValue=="Register_Type_Name")
          {
              if(item.Register_Type_Name.includes(search))
              {
                return item;
              }
              }else if(this.optionValue=="Register_Type_Code"){
                   var Snumber=item.Register_Type_Code.toString();
