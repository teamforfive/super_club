<template>
 ...
 <div>
    <el-table
      ...
      <el-table-column align="center"   label="省份编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Province_Code }}
        </template>
      </el-table-column>
       <el-table-column align="center"   label="省份名称" width="120" fixed>
        <template slot-scope="scope">
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_Province'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  data() {
    return {
      ...
      value:'',
       rules: {
        Province_Name: [{ required: true, message: '省份名称必填！', trigger: 'blur' }],
    ...
    var _this=this;
      ...
      getList().then(response => {
        ...
        if (search) {
          ...
          if(this.optionValue=="Province_Name")
          {
              if(item.Depart_Code.includes(search))
              {
                return item;
              }
              }else if(this.optionValue=="Province_Code"){
