<template>
 ...
 <div>
    <el-table
      ...
      <el-table-column align="center"   label="人员类别编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Person_Type_Code}}
        </template>
      </el-table-column>
       <el-table-column align="center"   label="人员类别名称" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Person_Type_Name }}
        </template>
      </el-table-column>
      ...
      <el-form label-width="140px" label-position="left" ref="form" :model="form" :rules="rules">
        ...
        <el-form-item label="人员类别名称" prop="Person_Type_Name">
          <el-input  v-model="form.Person_Type_Name" ></el-input>
        </el-form-item>
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_Person_Type'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  data() {
    return {
      ...
      value:'',
       rules: {
        Person_Type_Name: [{ required: true, message: '人员类别名称必填！', trigger: 'blur' }],
    ...
    var _this=this;
      ...
      getList().then(response => {
        ...
        if (search) {
          ...
          if(this.optionValue=="Person_Type_Name")
          {
              if(item.Person_Type_Name.includes(search))
              {
                return item;
              }
              }else if(this.optionValue=="Person_Type_Code"){
                   var Snumber=item.Person_Type_Code.toString();
