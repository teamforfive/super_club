<template>
 ...
 <div>
    <el-table
      ...
      <el-table-column align="center"   label="血型编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Blood_Type_Code}}
        </template>
      </el-table-column>
       <el-table-column align="center"   label="血型名称" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Blood_Type_Name}}
        </template>
      </el-table-column>
      ...
      <el-form label-width="140px" label-position="left" ref="form" :model="form" :rules="rules">
        ...
        <el-form-item label="血型名称" prop="Blood_Type_Name">
          <el-input  v-model="form.Blood_Type_Name" ></el-input>
        </el-form-item>
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_Blood_Type'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  data() {
    return {
      ...
      dialogType: 'yes',
      options: [{
          value: 'Blood_Type_Name',
          label: '血型名称'
        }, {
          value: 'Blood_Type_Code',
          label: '血型编码'
        },],
        search:'',
        optionValue:'',
      ...
      value:'',
       rules: {
        Blood_Type_Name: [{ required: true, message: '血型名称必填！', trigger: 'blur' }],
    ...
    var _this=this;
      ...
      getList().then(response => {
        ...
        if (search) {
          ...
          if(this.optionValue=="Blood_Type_Name")
          {
              if(item.Blood_Type_Name.includes(search))
              {
                return item;
              }
              }else if(this.optionValue=="Blood_Type_Code"){
                   var Snumber=item.Blood_Type_Code.toString();
