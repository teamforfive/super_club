<template>
 ...
 <div>
    <el-table
      ...
      <el-table-column align="center"   label="挂号来源编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Register_Source_Code}}
        </template>
      </el-table-column>
       <el-table-column align="center"   label="挂号来源名称" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Register_Source_Name }}
        </template>
      </el-table-column>
      ...
      <el-form label-width="140px" label-position="left" ref="form" :model="form" :rules="rules">
        ...
        <el-form-item label="挂号来源名称" prop="Register_Source_Name">
          <el-input  v-model="form.Register_Source_Name" ></el-input>
        </el-form-item>
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_Register_Source'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  data() {
    return {
      ...
      dialogType: 'yes',
      options: [{
          value: 'Register_Source_Name',
          label: '挂号来源名称'
        }, {
          value: 'Register_Source_Code',
          label: '挂号来源编码'
        },],
        search:'',
        optionValue:'',
      ...
      value:'',
       rules: {
        Register_Source_Name: [{ required: true, message: '挂号来源名称必填！', trigger: 'blur' }],
  ...
  methods: {
    ...
    handleAdd() {
      this.form.Register_Source_Name='',
      this.form.Sort_Id='';
      this.Is_Enable='';
      this.Is_Delete='';
      this.form.Is_Delete='';
    ...
    var _this=this;
      ...
      getList().then(response => {
        ...
        if (search) {
          ...
          if(this.optionValue=="Register_Source_Name")
          {
              if(item.Register_Source_Name.includes(search))
              {
                return item;
              }
              }else if(this.optionValue=="Register_Source_Code"){
                   var Snumber=item.Register_Source_Code.toString();
