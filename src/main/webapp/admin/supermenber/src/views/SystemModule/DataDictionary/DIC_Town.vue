<template>
 ...
 <div>
    <el-table
      ...
      <el-table-column align="center"   label="镇、街道编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Town_Code}}
        </template>
      </el-table-column>
       <el-table-column align="center"   label="镇、街道名称" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Town_Name }}
        </template>
      </el-table-column>
      ...
      <el-form label-width="140px" label-position="left" ref="form" :model="form" :rules="rules">
        ...
        <el-form-item label="镇、街名称" prop="Town_Name">
          <el-input  v-model="form.Town_Name" ></el-input>
        </el-form-item>
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_Town'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  data() {
    return {
      ...
      value:'',
       rules: {
        Town_Name: [{ required: true, message: '镇、街名称必填！', trigger: 'blur' }],
    ...
    var _this=this;
      ...
      getList().then(response => {
        ...
        if (search) {
          ...
          if(this.optionValue=="Town_Name")
          {
              if(item.Town_Name.includes(search))
              {
                return item;
              }
              }else if(this.optionValue=="Town_Code"){
