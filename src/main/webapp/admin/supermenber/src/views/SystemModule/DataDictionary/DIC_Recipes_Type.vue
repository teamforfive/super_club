<template>
 ...
 <div>
    <el-table
      ...
      <el-table-column align="center"   label="处方类别编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Recipes_Type_Code}}
        </template>
      </el-table-column>
       <el-table-column align="center"   label="处方类别名称" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Recipes_Type_Name }}
        </template>
      </el-table-column>
      ...
      <el-form label-width="140px" label-position="left" ref="form" :model="form" :rules="rules">
        ...
        <el-form-item label="处方类别名称" prop="Recipes_Type_Name">
          <el-input  v-model="form.Recipes_Type_Name" ></el-input>
        </el-form-item>
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_Recipes_Type'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  data() {
    return {
      ...
      dialogType: 'yes',
      options: [{
          value: 'Recipes_Type_Name',
          label: '处方类别名称'
        }, {
          value: 'Recipes_Type_Code',
          label: '处方类别编码'
        },],
        search:'',
        optionValue:'',
      ...
      value:'',
       rules: {
        Recipes_Type_Name: [{ required: true, message: '处方类别名称必填！', trigger: 'blur' }],
    ...
    var _this=this;
      ...
      getList().then(response => {
        ...
        if (search) {
          ...
          if(this.optionValue=="Recipes_Type_Name")
          {
              if(item.Recipes_Type_Name.includes(search))
              {
                return item;
              }
              }else if(this.optionValue=="Recipes_Type_Code"){
                   var Snumber=item.Recipes_Type_Code.toString();
