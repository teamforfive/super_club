<template>
 ...
 <div>
    <el-table
      ...
      <el-table-column align="center"   label="婚姻编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Marriage_Code}}
        </template>
      </el-table-column>
       <el-table-column align="center"   label="婚姻名称" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Marriage_Name }}
        </template>
      </el-table-column>
      ...
      <el-form label-width="140px" label-position="left" ref="form" :model="form" :rules="rules">
        ...
        <el-form-item label="婚姻名称" prop="Marriage_Name">
          <el-input  v-model="form.Marriage_Name" ></el-input>
        </el-form-item>
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_Marriage'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  data() {
    return {
      ...
      value:'',
       rules: {
        Marriage_Name: [{ required: true, message: '婚姻名称必填！', trigger: 'blur' }],
    ...
    var _this=this;
      ...
      getList().then(response => {
        ...
        if (search) {
          ...
          if(this.optionValue=="Marriage_Name")
          {
              if(item.Marriage_Name.includes(search))
              {
                return item;
              }
              }else if(this.optionValue=="Marriage_Code"){
                  var Snumber=item.Marriage_Code.toString();
