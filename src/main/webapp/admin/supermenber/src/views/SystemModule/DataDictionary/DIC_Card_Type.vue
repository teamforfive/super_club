<template>
 ...
 <div>
    <el-table
      ...
      <el-table-column align="center"   label="编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Card_Type_Code}}
        </template>
      </el-table-column>
       <el-table-column align="center"   label="名称" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Card_Type_Name }}
        </template>
      </el-table-column>
      ...
      <el-form label-width="140px" label-position="left" ref="form" :model="form" :rules="rules">
        ...
        <el-form-item label="名称" prop="Card_Type_Name">
          <el-input  v-model="form.Card_Type_Name" ></el-input>
        </el-form-item>
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_Card_Type'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  data() {
    return {
      ...
      value:'',
       rules: {
        Card_Type_Name: [{ required: true, message: '名称必填！', trigger: 'blur' }],
    ...
    var _this=this;
      ...
      getList().then(response => {
        ...
        if (search) {
          ...
          if(this.optionValue=="Card_Type_Name")
          {
              if(item.Card_Type_Name.includes(search))
              {
                return item;
              }
              }else if(this.optionValue=="Card_Type_Code"){
                  var Snumber=item.Card_Type_Code.toString();
