<template>
 ...
 <div>
    <el-table
      ...
      style="width: 100%"
       max-height="550" >
      <el-table-column align="center"   label="疾病诊断编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.Diagnosis_Code }}
        </template>
      </el-table-column>
      <el-table-column label="疾病诊断名称" width="120">
        <template slot-scope="scope">
          {{ scope.row.Diagnosis_Name }}
        </template>
      </el-table-column>
      <el-table-column label="ICD编码" width="120" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.ICD_Code }}</span>
        </template>
      </el-table-column>
      ...
      <el-form label-width="140px" label-position="left" ref="form" :model="form" :rules="rules">
        <el-form-item label="疾病诊断编码" prop="Diagnosis_Code">
          <el-input  placeholder="Role Name" v-model="form.Diagnosis_Code" />
        </el-form-item>
        <el-form-item label="疾病诊断名称" prop="Diagnosis_Name">
          <el-input  placeholder="Role Name" v-model="form.Diagnosis_Name" />
        </el-form-item>
        ...
        <el-form-item label="排序码" prop="Sort_Id">
          <el-input type="ICD_Code" v-model.number="form.ICD_Code"  autocomplete="off"></el-input>
        </el-form-item>
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_Diagnosis_Chinese'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  data() {
    return {
      ...
      value:'',
       rules: {
        Diagnosis_Name: [{ required: true, message: '疾病诊断必填！', trigger: 'blur' }],
        ICD_Code: [{ required: true, message: '排序码必填！', trigger: 'blur' },{ type: 'number', message: '必须为数字值'}],
    ...
    var _this=this;
      ...
      getList().then(response => {
        ...
        if (search) {
          ...
          if(this.optionValue=="Diagnosis_Name")
          {
              if(item.Diagnosis_Name.includes(search))
              {
                return item;
              }
              }else if(this.optionValue=="Diagnosis_Code"){
                   var Snumber=item.Diagnosis_Code.toString();
