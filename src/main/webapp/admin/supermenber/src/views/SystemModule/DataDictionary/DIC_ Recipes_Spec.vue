<template>
 ...
 <div>
    <el-table
      ...
      <el-table-column align="center"   label="处方种类编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Recipes_Spec_Code}}
        </template>
      </el-table-column>
       <el-table-column align="center"   label="处方种类名称" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Recipes_Spec_Name }}
        </template>
      </el-table-column>
      ...
      <el-form label-width="140px" label-position="left" ref="form" :model="form" :rules="rules">
        ...
        <el-form-item label="处方种类名称" prop="Recipes_Spec_Name">
          <el-input  v-model="form.Recipes_Spec_Name" ></el-input>
        </el-form-item>
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_ Recipes_Spec'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  data() {
    return {
      ...
      value:'',
       rules: {
        Recipes_Spec_Name: [{ required: true, message: '处方种类名称必填！', trigger: 'blur' }],
    ...
    var _this=this;
      ...
      getList().then(response => {
        ...
        if (search) {
          ...
          if(this.optionValue=="Recipes_Spec_Name")
          {
              if(item.Recipes_Spec_Name.includes(search))
              {
                return item;
              }
              }else if(this.optionValue=="Recipes_Spec_Code"){
                   var Snumber=item.Recipes_Spec_Code.toString();
