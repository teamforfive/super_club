<template>
 ...
 <div>
    <el-table
      ...
      <el-table-column align="center"   label="关系编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Relation_Code}}
        </template>
      </el-table-column>
       <el-table-column align="center"   label="关系名称" width="120" fixed>
        <template slot-scope="scope">
      ...
      <el-form label-width="140px" label-position="left" ref="form" :model="form" :rules="rules">
        ...
        <el-form-item label="关系名称" prop="Relation_Name">
          <el-input  v-model="form.Relation_Name" ></el-input>
        </el-form-item>
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_Relation'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  data() {
    return {
      ...
      value:'',
       rules: {
        Relation_Name: [{ required: true, message: '关系名称必填！', trigger: 'blur' }],
    ...
    var _this=this;
      ...
      getList().then(response => {
        ...
        if (search) {
          ...
          if(this.optionValue=="Relation_Name")
          {
              if(item.Relation_Name.includes(search))
              {
                return item;
              }
              }else if(this.optionValue=="Rh_Code"){
                   var Snumber=item.Relation_Code.toString();
