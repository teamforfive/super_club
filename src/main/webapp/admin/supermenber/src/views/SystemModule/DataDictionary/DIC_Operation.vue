<template>
 ...
 <div>
    <el-table
      ...
      <el-table-column align="center"   label="手术编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.Code }}
        </template>
      </el-table-column>
      <el-table-column label="手术名称" width="120">
        <template slot-scope="scope">
          {{ scope.row.Name }}
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      value:'',
       rules: {
        Name: [{ required: true, message: '手术名称必填！', trigger: 'blur' }],
        ICD_Code: [{ required: true, message: 'ICD编码必填！', trigger: 'blur' },{ type: 'number', message: '必须为数字值'}],
