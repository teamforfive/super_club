<template>
 ...
 <div>
    <el-table
      ...
      <el-table-column align="center"   label="市编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.City_Code }}
        </template>
      </el-table-column>
       <el-table-column align="center"   label="市名称" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.City_Name }}
        </template>
      </el-table-column>
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_City'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  data() {
    return {
      ...
      value:'',
       rules: {
        City_Name: [{ required: true, message: '市名称必填！', trigger: 'blur' }],
    ...
    var _this=this;
      ...
      getList().then(response => {
        ...
        if (search) {
          ...
          if(this.optionValue=="City_Name")
          {
              if(item.City_Name.includes(search))
              {
                return item;
              }
              }else if(this.optionValue=="City_Code"){
                  var Snumber=item.City_Code.toString();
