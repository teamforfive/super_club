      ...
      <el-form label-width="140px" label-position="left" ref="form" :model="form" :rules="rules">
        ...
        <el-form-item label="床位编码" prop="Bed_Code">
         ...
         <el-form-item label="位置" prop="Seat">
          <el-input  placeholder="Role Name" v-model="form.Seat" />
        </el-form-item>
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_Bed'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
    ...
    return {
      ...
      value:'',
       rules: {
        ...
        Seat: [{ required: true, message: '位置必填！', trigger: 'blur' }],
          ...
          if(this.optionValue=="Depart_Code")
          {
              if(item.Depart_Code.includes(search))
              {
                return item;
                 ...
                 var Snumber=item.Bed_Code.toString();
