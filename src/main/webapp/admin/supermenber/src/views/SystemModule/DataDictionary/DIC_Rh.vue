<template>
 ...
 <div>
    <el-table
      ...
      <el-table-column align="center"   label="Rh编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Rh_Code}}
        </template>
      </el-table-column>
       <el-table-column align="center"   label="Rh名称" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.row.Rh_Name }}
        </template>
      </el-table-column>
      ...
      <el-form label-width="140px" label-position="left" ref="form" :model="form" :rules="rules">
        ...
        <el-form-item label="Rh名称" prop="Rh_Name">
          <el-input  v-model="form.Rh_Name" ></el-input>
        </el-form-item>
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_Rh'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  data() {
    return {
      ...
      value:'',
       rules: {
        Rh_Name: [{ required: true, message: 'Rh名称必填！', trigger: 'blur' }],
    ...
    var _this=this;
      ...
      getList().then(response => {
        ...
        if (search) {
          ...
          if(this.optionValue=="Rh_Name")
          {
              if(item.Rh_Name.includes(search))
              {
                return item;
              }
              }else if(this.optionValue=="Rh_Code"){
                   var Snumber=item.Rh_Code.toString();
