<template>
  ...
  <el-input class="inputiner"
    placeholder="请输入内容"
    v-model="search" clearable size="mini">
    <i slot="prefix" class="el-input__icon el-icon-search"></i>
  </el-input>
 ...
 <div>
    <el-table
      ...
      <el-table-column align="center"   label="科室编码" width="120" fixed>
        <template slot-scope="scope">
          {{ scope.Dept_Code }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="科室类型" width="120" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.Dept_Class }}</span>
        </template>
      </el-table-column>
      <el-table-column label="显示排序号" width="120" align="center">
        <template slot-scope="scope">
          {{ scope.row.Sort_Id }}
        </template>
      </el-table-column>
      <el-table-column label="上级科室编码" width="120" align="center">
        <template slot-scope="scope">
      ...
      <el-form label-width="140px" label-position="left" ref="form" :model="form" :rules="rules">
        <el-form-item label="科室名称" prop="Dept_Name">
          <el-input  placeholder="Role Name" v-model="form.Dept_Name" />
        </el-form-item>
        <el-form-item label="科室类型" prop="Dept_Class">
          <el-input  placeholder="Role Name" v-model="form.Dept_Class" />
        </el-form-item>
         <el-form-item label="显示排序号" prop="Sort_Id">
          <el-input type="Sort_Id" v-model.number="form.Sort_Id"  autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="上级科室编码" prop="Parent_Code">
           <el-input type="Parent_Code" v-model.number="form.Parent_Code"  autocomplete="off"></el-input>
        </el-form-item>
         <el-form-item label="备注" prop="Notes">
          <el-input  placeholder="Role Name" v-model="form.Notes" />
        </el-form-item>
      <el-form-item label="是否可用" prop="Is_Enable">
        ...
        <el-form-item label="是否可删除" prop="Is_Delete">
         ...
         <el-form-item label="创建时间" prop="Create_date">
          <el-input  placeholder="Role Name" v-model="form.Create_date" />
        </el-form-item>
...
<script>
import { getList, addList,UdList} from '@/api/DataDictionary/DIC_Department'
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  data() {
    return {
      ...
      options: [{
        ...
        value:'',
        rules: {
        Dept_Name: [{ required: true, message: '科室名称必填！', trigger: 'blur' }],
        Dept_Class: [{ required: true, message: '科室类型名称必填！', trigger: 'blur' }],
        Parent_Code: [{ required: true, message: '上级科室编码必填！', trigger: 'blur' },{ type: 'number', message: '必须为数字值'}],
    ...
    var _this=this;
      ...
      getList().then(response => {
        ...
        if (search) {
          ...
          if(this.optionValue=="Dept_Name")
          {
 					if(item.City_Name.includes(search))
 					{
 						return item;
           }
          }else if(this.optionValue=="Dept_Code"){
              var Snumber=item.Dept_Code.toString();
